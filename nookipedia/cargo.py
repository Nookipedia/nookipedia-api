...
def get_villager_list(limit, tables, join, fields):
    ...
    if request.args.get('personality'):
        ...
        if personality not in personality_list:
            abort(400, description=error_response("Could not recognize provided personality.",
    ...
    if request.args.get('species'):
        species_list = ['alligator', 'anteater', 'bear', 'bear cub', 'bird', 'bull', 'cat', 'cub', 'chicken', 'cow',
                        'deer', 'dog', 'duck', 'eagle', 'elephant', 'frog', 'goat', 'gorilla', 'hamster', 'hippo',
                        'horse', 'koala', 'kangaroo', 'lion', 'monkey', 'mouse', 'octopus', 'ostrich', 'penguin', 'pig',
                        'rabbit', 'rhino', 'rhinoceros', 'sheep', 'squirrel', 'tiger', 'wolf']
        species = request.args.get('species').lower()
        if species not in species_list:
            abort(400, description=error_response("Could not recognize provided species.",
                                                  "Ensure provided species is valid."))
    ...
    params = {'action': 'cargoquery', 'format': 'json', 'limit': limit, 'tables': tables, 'join_on': join,
              'fields': fields}
...
def get_furniture_list(limit, tables, fields):
    ...
    if 'category' in request.args:
        ...
        if category not in categories_list:
            abort(400, description=error_response('Could not recognize provided category.',
...
def get_furniture_variation_list(limit, tables, fields, orderby):
    ...
    if 'color' in request.args:
        colors_list = ['aqua', 'beige', 'black', 'blue', 'brown', 'colorful', 'gray', 'green', 'orange', 'pink',
                       'purple', 'red', 'white', 'yellow']
        colors = [color.lower() for color in request.args.getlist('color')]
        for color in colors:
            if color not in colors_list:
                abort(400, description=error_response('Could not recognize provided color.',
        ...
        if len(colors) == 1:  # If they only filtered one color
            ...
            where.append(
                '((color1 = "{0}" AND color2 = "{1}") OR (color1 = "{1}" AND color2 = "{0}"))'.format(colors[0],
                                                                                                      colors[1]))
        else:
    ...
    params = {'action': 'cargoquery', 'format': 'json', 'tables': tables, 'fields': fields, 'order_by': orderby,
              'limit': limit}
    params_where(params, where)
