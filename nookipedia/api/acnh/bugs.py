...
from nookipedia.config import DB_KEYS, BUG_LIMIT
...
router = Blueprint("bugs", __name__)


# All New Horizons bugs
@router.route("/nh/bugs", methods=["GET"])
def get_nh_bug_all():
...
@router.route("/nh/bugs/<string:bug>", methods=["GET"])
def get_nh_bug(bug):
    authorize(DB_KEYS, request)
